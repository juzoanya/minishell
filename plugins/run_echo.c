...
/*   Updated: 2022/09/12 11:53:53 by juzoanya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	call_run_echo(t_mini *shell, int n_flag)
{
	int	i;
	int	n;
	...
	if (count_args(shell->exec_args) > 1)
	{
		while (shell->exec_args[i] != NULL
			&& (ft_strncmp(shell->exec_args[i], "-n", 2) == 0
					|| (ft_strncmp(shell->exec_args[i], "-n", 2) == 0 && count_args(shell->exec_args) > 2)))
		{
			n_flag = 1;
			i++;
		}
